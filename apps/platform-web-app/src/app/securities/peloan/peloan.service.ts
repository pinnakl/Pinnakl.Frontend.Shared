...
export class PeloanService {
  ...
  async getPeLoanDetailsById(id: number): Promise<PeLoan> {
    const peLoans = await this.wsp.getHttp<PeLoanApi[]>({
      endpoint: this._peLoanEndpoint,
      params: {
        fields: ['*'],
        filters: [
    ...
    return peLoans && peLoans.length > 0
      ? this.formatPeLoanApi(peLoans[0])
      : null;
  }
  ...
  async getPeLoanFromSecurityId(securityId: number): Promise<PeLoan> {
    ...
    const peLoan = await this.wsp.getHttp<PeLoanApi[]>({
      endpoint: this._peLoanEndpoint,
      params: {
        fields: ['*'],
        filters: [
    ...
    return peLoan && peLoan.length > 0 ? this.formatPeLoanApi(peLoan[0]) : null;
  }
  ...
  formatPeLoanApi(entity: PeLoanApi): PeLoan {
    return {
      ...
      interestBasisId: entity.interestbasisid ? parseInt(entity.interestbasisid, 10) : null,
